...
#
# Tags look like `v<Cinny version>-<release>`:
#
# - if defaults/main.yml points at a Cinny version that has never been
#   released, the release counter restarts at 0 (`v4.12.6-0`)
# - otherwise the counter is incremented (`v4.12.6-1`), but only if something
...
version="$(sed -nE 's|^cinny_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Cinny version from $defaults_path"
	exit 1
fi

# Cinny's version values carry a leading `v` (e.g. `v4.12.6`) and so do the
# tags (`v4.12.6-0`). Stripping any `v` before prepending one keeps this
# correct even if the version values ever stop carrying one.
